...
void solve() {
	...
	set<int> unassigned(all(poi));

    mn ans = 1;

    mn peri = 0;
	auto assign_range = [&](int L, int R, int H) {
		auto it = unassigned.lb(L);
		while (it != unassigned.end() && *it < R) {
			int inx = lb(all(poi), *it) - poi.begin();

			assert(inx > 0);
			assert(inx + 1 < sz(poi));

			peri -= abs(ht[inx] - ht[inx - 1]);
			peri -= abs(ht[inx] - ht[inx + 1]);
