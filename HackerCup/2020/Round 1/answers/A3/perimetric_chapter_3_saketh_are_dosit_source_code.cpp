...
void solve() {
    ...
    auto count_zero = [&](int L, int R) {
        int ans = 0;
        auto it = prev(ht.upper_bound(L));
        for (; it != ht.end() && it->f < R; it++) {
            if (it->s == 0) {
                int iL = max(L, it->f);
                int iR = min(R, next(it) == ht.end() ? INT_MAX : next(it)->f);
                ans += iR - iL;
            }
        }
        return ans;
    ...
    auto count_vert = [&](int L, int R) {
        mn ans = 0;
        for (auto it = ht.lb(L); it != ht.end() && it->f <= R; it++) {
            ans += abs(it->s - prev(it)->s);
        }
        return ans;
    };

    auto get_cur = [&](int x) {
        auto it = ht.upper_bound(x);
        return prev(it)->s;
    };

    auto assign_range = [&](int L, int R, int H) {
        peri += 2 * count_zero(L, R);

        int cur_bef = get_cur(L - 1);
        int cur_aft = get_cur(R);

        peri -= count_vert(L, R);

        // clear anything in [L, R)
        for (auto it = ht.lb(L); it != ht.end() && it->f < R; ) {
            ht.erase(it++);
        }

        peri += abs(H - cur_bef) + abs(H - cur_aft);
